...
| Phase | 日付 | タイトル | 主な成果 |
|-------|------|---------|---------|
| 24 | 2026-02-11 | 壁打ちチャット修正 + SME判定テーブル + ドラフト強化 + テスト基盤 | 壁打ちチャット無限スピナー修正(テンプレートリテラルJS構文エラー), 中小企業基本法準拠SME判定テーブル(4業種×3閾値), ドラフト動的セクション12種(受給要件/スケジュール追加), NGルール6→21件拡張, Vitest導入+73テスト全通過, AbortController15秒タイムアウト |
| 23 | 2026-02-11 | 検索パフォーマンス・精度強化・TODO体系化 | Promise.all並列化(3箇所), トークン分割AND検索, 都道府県フィルターUI, FNV-1aキャッシュキー, KPI計測(perf_ms), SEC-1修正(debug情報本番ガード), DO_NOT_PROCEED統一, TODO:要確認25件体系化(受入基準/テストケース付), IMPROVEMENT_PLAN.md作成 |
| 22 | 2026-02-11 | E2Eフロー検証・BUG修正・設計分析 | E2Eデータフロー完全マッピング, BUG-1(/api/drafts 404修正), BUG-2(ダッシュボードドラフトリンク修正), BUG-4(壁打ち再開時概要パネル修正), AIコンシェルジュ×ドラフト整合性評価, UX欠陥5件洗い出し, コード品質チェック, テストケース4件定義, フロー図作成 |
...
## Phase 24: 壁打ちチャット修正 + SME判定テーブル + ドラフト強化 + テスト基盤 (2026-02-11)

### 計画
- 壁打ちチャットの無限スピナー問題修正（最優先）
- Phase 23で策定したIMPROVEMENT_PLANの推奨タスク実施
  - MATCH-1/2/3: 中小企業基本法に基づく業種別SME判定テーブル
  - DRAFT-1: NormalizedSubsidyDetailからの動的セクション生成
  - SEARCH-1: トークン化検索テストケース20件
  - OPS-1: E2Eテストセットアップ（Vitest）

### 実施内容

**1. 壁打ちチャット無限スピナー修正**
- **根本原因**: `c.html()` テンプレートリテラル内のJS文字列で `\n` が実際の改行文字に展開されシングルクォート文字列が壊れる
- **修正1**: `addMessage` 呼び出しの `\n` を `\\n` にダブルエスケープ（文字列連結方式）
- **修正2**: `askSuggested` onclick内のクォートエスケープを `&apos;` HTMLエンティティに変更
- **修正3**: `window.api` に AbortController 15秒タイムアウト追加
- **検証**: 全9ページの<script>タグをvm.Script構文チェック → 0エラー

**2. MATCH-1/2/3: 中小企業基本法準拠SME判定テーブル**
- `SME_THRESHOLDS`: 4業種（manufacturing/wholesale/service/retail）× 3閾値（資本金/従業員/小規模）
- `resolveIndustryCategory()`: industry_major文字列 → SmeIndustryCategory変換（完全一致+部分一致）
- `isSmeByLaw()`: OR条件判定（資本金上限以下 OR 従業員上限以下 → 中小企業）
- `checkEmployeeMatchV2` / `checkCapitalV2` を業種別テーブル使用に改修
- テスト: `tests/screening-sme.test.ts` 37テスト全通過

**3. DRAFT-1: ドラフト動的セクション強化**
- 受給要件セクション追加: `eligibility_rules` → カテゴリ別グループ化 + 自社合致状況自動判定
- 申請スケジュールセクション追加: `acceptance` → 締切残日数計算 + 推奨準備スケジュール
- セクション総数: 基本5 + 動的7 = 最大12セクション
- NGルール拡張: 6 → 21ルール（9カテゴリ: 断定/誇張/不正/審査注意/あいまい/差別/批判/依存/法令）

**4. SEARCH-1 + OPS-1: テスト基盤構築**
- Vitest導入 (`vitest ^4.0.18`)
- `tests/tokenize-search.test.ts`: トークン化検索20テストケース
- `tests/screening-sme.test.ts`: SME判定37テストケース
- `tests/ng-rules.test.ts`: NGルール16テストケース
- **合計73テスト全通過**

### 結果
| 指標 | Before | After |
|------|--------|-------|
| チャットページ構文エラー | 1件 (Invalid or unexpected token) | 0件 |
| SME判定 | 一律（製造業基準のみ） | 4業種別テーブル |
| ドラフトセクション | 最大10種 | 最大12種（受給要件/スケジュール追加） |
| NGルール | 6ルール | 21ルール |
| テストケース | 0件 | 73件 |

### 次Phase推奨
- SEC-2: debugパラメータ権限チェック
- SEC-3: ドラフト更新バリデーション（XSS/SQLi）
- CACHE-1/2: キャッシュTTL最適化・無効化戦略
- DRAFT-3: 電子申請案内自動生成の詳細化
- DRAFT-4: LLM統合ドラフト生成

---

## Phase 23: 検索パフォーマンス・精度強化・TODO体系化 (2026-02-11)

### 計画
Phase 22のE2E分析で発覚した検索パフォーマンス課題、マッチング精度問題、セキュリティ懸念に対処。
TODO: 要確認タグを体系化し、受け入れ基準・テストケース付きで管理する仕組みを確立。

### 実施内容

#### 1. パフォーマンス改善（5箇所の並列化）
- `getCompanySSOT.ts`: 3つの逐次DBクエリ → Promise.all (推定66%削減)
- `getNormalizedSubsidyDetail.ts`: canonical/snapshot/cache取得を並列化 (推定66%削減)
- `subsidies.ts`: adapter.search + getCompanySSOT を同時実行 (推定50%削減)
- `subsidies.ts`: NSD変換を for-loop → Promise.all (推定80%削減)
- `subsidies.ts`: FNV-1aハッシュによるキャッシュキー衝突率低減

#### 2. 検索機能強化
- トークン分割AND検索（SSOT/cache両バックエンド）
- issuer_name を検索対象に追加
- 都道府県フィルター（47都道府県+全国のUIドロップダウン）
- 全角スペース→半角正規化、最大5トークン制限（DoS対策）

#### 3. KPI計測の実装
- usage_eventsに `perf_ms.total / search_and_ssot / nsd_normalize` を記録
- レスポンスmetaに `perf_total_ms` を追加（フロント側体感把握用）
- prefectureフィルター利用率も記録開始

#### 4. セキュリティ修正（SEC-1）
- 本番環境（ENVIRONMENT=production or JGRANTS_MODE=cached-only）でdebug_message/debug_stackを返さないガード追加

#### 5. バグ修正
- DO_NOT_PROCEED の士業ダッシュボードバッジ対応（BUG-10）
- ステータスフィルター・カウントのDO_NOT_PROCEED統一（BUG-11/12）

#### 6. TODO: 要確認 体系化（25件）
- 全8ファイルに // TODO: 要確認 を挿入（受け入れ基準・テストケース付き）
- カテゴリ分類: A:セキュリティ(3件), B:パフォーマンス(3件), C:検索精度(3件), D:マッチング(3件), E:SSOT(5件), F:ドラフト(8件)
- IMPROVEMENT_PLAN.md 作成（フェーズ別ロードマップ・リスク分析付き）

### 成果
- パフォーマンス: 検索処理全体で推定60-70%の遅延削減
- 機能: トークン分割検索・都道府県フィルター・KPI計測が稼働
- セキュリティ: SEC-1修正（本番debug情報非露出）
- 管理: TODO 25件を受け入れ基準付きで体系化、改善計画ドキュメント作成

### 次のアクション（Phase 24）
1. MATCH-1/2/3: 業種別の中小企業判定テーブル実装
2. DRAFT-1: 動的セクション生成
3. SEARCH-1: トークン分割検索のテスト20件
4. OPS-1: E2Eテスト整備
...
## Phase 19-AI: AIコンシェルジュ壁打ちチャット (2026-02-11)

### 計画
壁打ちチャットを「固定質問→回答→次の質問」のインタビュー形式から、AIコンシェルジュに相談できる対話体験へ進化させる。

### 実施内容

#### 1. AIコンシェルジュエンジン (src/lib/ai-concierge.ts)
- OpenAI互換API連携（gpt-5-mini）
- 動的システムプロンプト生成:
  - 補助金のNormalizedSubsidyDetail（要件・対象経費・書類・加点項目）を注入
  - 企業情報（業種・従業員数・資本金・地域）を反映
  - 収集済みfactsを反映（過去の回答を踏まえた文脈）
  - 会話履歴（最新10件）を保持
- 2つのモード:
  - **structured**: 構造化質問フェーズ。AIが自然な会話で情報を引き出す
  - **free**: コンシェルジュ相談モード。補助金申請の何でも相談
- フォールバック応答: APIキーなしでも動作（要件/書類/金額/締切への定型応答）
- usage_eventsにトークン使用量を記録（コスト管理）

#### 2. チャットAPI改修 (src/routes/chat.ts)
- `POST /api/chat/sessions/:id/message` を全面改修
  - 構造化質問フェーズ: AIが前の回答に反応→次の質問を自然に聞く
  - 構造化質問完了→自動的にconsultingモードに移行
  - completedセッションからconsultingへの復帰対応
  - レスポンスに`mode`, `consulting_mode`, `suggested_questions`を追加
- セッション状態: `collecting` → `consulting`（新ステータス）

#### 3. UI大幅刷新 (src/pages/chat.tsx)
- コンシェルジュモード切り替えUI:
  - 構造化質問完了時に「AIに相談」+「申請書作成」の2つのボタン
  - consultingモード専用入力エリア
- 提案質問ボタン: AIが生成した関連質問をワンタップで送信
- クイック相談ボタン: 「申請要件を確認」「必要書類を確認」
- セッション復元時のconsultingモード対応

#### 4. 環境設定
- `OPENAI_BASE_URL` を Env型・.dev.vars・wrangler secretに追加
- 本番環境: OPENAI_API_KEY（既設）+ OPENAI_BASE_URL（新設）

### 成果
- 4ファイル変更、+902行/-49行
- 壁打ちの体験が「固定質問→回答」から「AI相談」に進化
- APIキーなしでもフォールバック応答で最低限の体験を維持
- 構造化質問と自由会話のハイブリッドで効率+自由度を両立

### 次アクション
1. 本番環境でのAI回答品質を検証（OPENAI_API_KEYが設定済み）
2. ストリーミング対応（SSE）でリアルタイム応答表示
3. 会話からの自動fact抽出（自由会話中に得た情報をfactsに保存）
4. プロンプトチューニング（採択率アップの実践的アドバイス）
5. 申請書ドラフト生成のAI化
...
## Phase 19: 補助金詳細ページ充実 + 壁打ちチャット機能強化 (2026-02-11)

### 計画
- 情報の泉の詳細情報を充実させる（概要タブのリッチ化）
- 壁打ちチャットの機能を充実させる（UI改善、対話フロー改善）

### 実施内容

#### Phase 19-A: 補助金詳細ページ概要タブリッチ化
- **概要表示改善**: summary を背景色+左ボーダー付きカードで表示
- **目的（purpose）セクション追加**: normalized.overview.purpose がある場合に青色カードで表示
- **対象事業セクション改善**: 紫色カードで統一的なデザイン
- **申請期間タイムライン追加**: 受付開始〜終了の進捗バーを表示、残日数のリアルタイム表示
- **電子申請情報セクション追加**: portal_name, portal_url, notes をリッチに表示
- **公式リンクセクション追加**: provenance.koubo_source_urls, pdf_urls をカード形式で表示
- **データソース情報追加**: ソース種別、スキーマバージョン、壁打ち準備状況を表示
- **基本情報カード拡張**: 4列→5列（壁打ち対応状況を追加）
- 変更ファイル: src/pages/subsidies.tsx

#### Phase 19-B: 壁打ちチャットUI全面刷新
- **補助金概要パネル**: チャット開始時に補助金名・概要・補助上限・補助率・締切を一覧表示
- **進捗バー**: 回答進捗をリアルタイムで可視化（X/Y件完了、パーセント表示）
- **質問カテゴリ表示**: 各質問にカテゴリバッジ（eligibility, document, profile等）
- **クイック回答の入力タイプ対応**: boolean→はい/いいえ、number→数値入力ヒント、text→自由記述ヒント
- **収集済み情報サマリー**: 折りたたみ式で回答済みの質問と回答を一覧表示
- **メッセージバブル改善**: アバターアイコン付き、アニメーション付きスライドイン
- **コンパクトなヘッダー**: スティッキーヘッダー、ステータス表示改善
- 変更ファイル: src/pages/chat.tsx（全面書き直し）

#### Phase 19-D: 壁打ち対話フロー改善
- **初期メッセージ充実**: 補助金概要・基本情報（上限額・補助率）を含む詳細な案内
- **応答メッセージ多様化**: 回答数に応じた励ましメッセージ（「順調に進んでいます」「もう少しです」等）
- **完了メッセージ改善**: 収集件数付きのサマリー、次のアクションの明確化
- 変更ファイル: src/routes/chat.ts

### 成果
- 補助金詳細ページ: 概要タブの情報量が大幅向上（6セクション追加）
- 壁打ちチャット: UX大幅改善（進捗可視化、補助金概要、収集情報サマリー）
- 対話フロー: より自然で情報量の多い対話体験
- 本番デプロイ済み: https://hojyokin.pages.dev

### 次アクション（Phase 20）
1. **izumi残りクロール**: fallback_v1 7,677件のバッチクロール継続
2. **壁打ち質問の品質向上**: izumi/manual系の補助金に対するwall_chat_questions生成
3. **Tokyo系Cron改善**: scrape-tokyoをDAILY_SYNC_JOBSに統合
4. **ダッシュボード**: Ready率推移グラフ、データ鮮度ダッシュボード
