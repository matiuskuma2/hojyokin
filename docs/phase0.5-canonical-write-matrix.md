...
| plans_wage_raise | PUT /profile (facts) | **x** | derived-questions (is_wage_raise_planned) | **x** | chat_facts のみ | **GAP: 士業 未対応** | 1b |
...
- [x] BUG-2 修正: `fact-sync.ts` の `is_wage_raise_planned` → `plans_to_hire` マッピング削除
