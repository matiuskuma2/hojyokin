...
|-----------|:---:|:---:|:---:|---|
| upload (POST) | o (POST /documents) | **POST /clients/:id/documents ✅** | **x** | 'direct' / 'agency' |
| list (GET) | o (GET /documents) | **GET /clients/:id/documents ✅** | **x** | - |
| delete (DELETE) | o (DELETE /documents/:id) | **DELETE /clients/:id/documents/:docId ✅** | **x** | - |
| text save | o (POST /documents/:id/text) | **x** (Phase 2b scope外) | **x** | - |
| extract | o (POST /documents/:id/extract) | **x** (Phase 2b scope外) | **x** | - |
| apply to profile | o (POST /documents/:id/apply) | **x** (Phase 2b scope外) | **x** | - |
...
士業 UI から顧客企業の書類（company_documents）を安全に upload / list / delete する。
Phase 2b のスコープは CRUD のみ。extract / apply / portal / intake フローには触れない。

### 変更ファイル

- `src/routes/agency/clients.ts` — 3 エンドポイント追加 + 共通権限チェック関数 + doc_type 正準値定義

### 追加エンドポイント

| method | path | 説明 |
|--------|------|------|
| GET | `/api/agency/clients/:id/documents` | 顧客の書類一覧（extracted_json, raw_text は除外） |
| POST | `/api/agency/clients/:id/documents` | 顧客の書類アップロード（multipart/form-data） |
| DELETE | `/api/agency/clients/:id/documents/:docId` | 顧客の書類削除（DB + R2 両方） |

### 設計決定

| 項目 | 決定 | 理由 |
|------|------|------|
| R2 key 命名 | `company-documents/{companyId}/{docId}/{filename}` | profile.ts L508 と統一 |
| doc_type 正準値 | `corp_registry`, `financials`, `tax_return`, `business_plan`, `other` | 不正値は `other` にフォールバック |
| 削除時の R2 連動 | DB + R2 両方削除。R2 失敗時は DB 削除を続行（ログ出力） | profile.ts L585-630 と統一。R2 orphan よりデータ整合性を優先 |
| R2 バインディング | `(c.env as any).R2` で安全アクセス | Env 型に `R2` 未定義。ローカル開発では R2 なしで動作 |
| 許可ファイル形式 | PDF, JPEG, PNG, WebP（10MB 上限） | profile.ts L499 と統一 |
| status 初期値 | `uploaded` | テーブルの DEFAULT と統一 |
| extract/apply | Phase 2b scope 外。GET で returned fields に status, confidence を含むのみ | 段階的実装の方針に従う |

### 権限チェック設計

```
agency_user → getUserAgency → agency.id
                                    ↓
agency_clients WHERE id=? AND agency_id=?
                                    ↓
                              client.company_id
                                    ↓
company_documents WHERE company_id=?
```

共通ヘルパー `resolveClientCompanyId(db, clientId, agencyId)` で集約。
全 3 エンドポイントで同一の権限パスを使用。

### テスト結果（全 11 テスト合格）

| # | テスト | 結果 |
|---|--------|------|
| 1 | GET documents (空一覧) | ✅ Total: 0 |
| 2 | POST upload PDF (corp_registry) | ✅ 201, status: uploaded, doc_type: corp_registry |
| 3 | POST upload JPEG (financials) | ✅ 201, doc_type: financials |
| 4 | GET documents (2件) | ✅ Total: 2, ファイル名・doc_type・status 正常 |
| 5 | DELETE document | ✅ deleted: true |
| 6 | GET after delete | ✅ Total: 1 |
| 7 | Invalid doc_type → fallback | ✅ doc_type: other |
| 8 | Fake client → 権限チェック | ✅ 404, NOT_FOUND |
| 9 | Fake document → 存在チェック | ✅ 404, NOT_FOUND |
| 10 | File too large (11MB) | ✅ 400, VALIDATION_ERROR |
| 11 | Disallowed file type (text/plain) | ✅ 400, VALIDATION_ERROR |

### 既存 route への影響

| route | 影響 |
|-------|------|
| profile.ts (法人UI) | **変更なし**。POST/GET/DELETE /documents は独立 |
| portal.ts | **変更なし** |
| submissions.ts | **変更なし** |
| getCompanySSOT.ts | **変更なし** |
| completeness.ts | **変更なし** |

### Phase 2b で追加しなかったもの

| 項目 | 理由 | 対応予定 |
|------|------|---------|
| text save (POST /documents/:id/text) | agency から raw_text を保存するユースケースがまだない | 必要時に追加 |
| extract (POST /documents/:id/extract) | 法人 UI の抽出ロジックを再利用するが、agency からの利用は Phase 3 以降 | Phase 3 |
| apply (POST /documents/:id/apply) | 抽出結果の自動反映は extract の後 | Phase 3 |
| uploaded_by / source 列追加 | DB スキーマ変更は最小限にする方針 | 必要時に migration 追加 |
| R2 バインディングの Env 型追加 | profile.ts も `c.env.R2` を型なしで使用中。全体の型整備は別タスク | 型整備タスク |

### 残課題

- **R2 バインディング型**: Env 型に `R2?: R2Bucket` を追加すべき（profile.ts + agency/clients.ts 両方で利用）
- **uploaded_by 列**: 誰がアップロードしたか追跡するには migration で列追加が必要
- **書類数の completeness 反映**: profile.ts の completeness では書類数ボーナスがあるが、agency 側の completeness.ts にはまだ含まれていない
- **GET /clients/:id に documents を含めるか**: 現在は別エンドポイント。統合する場合は構造化レスポンスに追加
