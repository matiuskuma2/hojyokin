...
- **profile.ts の fact_keys リストとの差異**: profile.ts は `['has_gbiz_id', 'is_invoice_registered', 'plans_wage_raise', 'tax_arrears', 'has_business_plan', 'has_keiei_kakushin', 'has_jigyou_keizoku']` の 7キー（`past_subsidy_same_type` が欠落）。Phase 1c で統一を検討。
