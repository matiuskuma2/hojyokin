...
-- 監視対象データソース（外部キー制約を緩和）
CREATE TABLE IF NOT EXISTS data_source_monitors (
  id TEXT PRIMARY KEY,
  subsidy_canonical_id TEXT,  -- NULLable - subsidy_canonical が存在しない場合もある
  subsidy_cache_id TEXT,  -- subsidy_cache との関連付け用
...
CREATE INDEX IF NOT EXISTS idx_data_source_monitors_cache ON data_source_monitors(subsidy_cache_id);
