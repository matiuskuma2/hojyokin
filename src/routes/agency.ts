 ...
 * 認証不要 - 招待リンクの有効性を確認
 */
agencyRoutes.post('/staff/verify-invite', async (c) => {
  const db = c.env.DB;
 ...
 * 認証不要 - 招待からパスワードを設定
 */
agencyRoutes.post('/staff/setup-password', async (c) => {
  const db = c.env.DB;
 ...
 */
agencyRoutes.get('/staff', async (c) => {
  const db = c.env.DB;
  const user = getCurrentUser(c);
 ...
 */
agencyRoutes.delete('/staff/:id', async (c) => {
  const db = c.env.DB;
  const user = getCurrentUser(c);
