 ...
 * 
 * // TODO: 要確認 - トークン分割検索の精度/再現率の定量評価
 *   現在AND検索で5トークン上限。部分一致がノイズになる可能性あり。
 *   受け入れ基準: 主要検索パターン20件で適合率70%以上、テストケース追加
 * // TODO: 要確認 - SSOT→cacheフォールバック時のログアラート設計
 *   現在console.errorのみ。運用監視でSSOT障害を即座に検知する仕組みが必要。
 *   受け入れ基準: フォールバック発生時にusage_eventsにも記録、アラートルール定義
 */
