...
- **Version**: 1.4.3 (PWA対応 + 運用監視強化)
- **Goal**: 企業情報を登録するだけで、最適な補助金・助成金を自動でマッチング＆申請書ドラフト作成

### 最新アップデート (v1.4.3) - PWA基盤 + 運用チェック

**スマホ最適化 + PWA対応 + 運用監視ダッシュボード**

1. **管理画面PWA対応**
   - Service Worker 登録（オフライン対応基盤）
   - Web App Manifest 設定済み
   - モバイルメニュー完全対応（ハンバーガー + ボトムナビ）

2. **運用チェックダッシュボード** (`/admin/ops`)
   - 30分検証チェックリスト（L1/L2/L3/KPI）
   - キュー健全性リアルタイム監視
   - ドメイン別エラー率Top10
   - 重複クロール検知
   - 今日のイベント（検索/壁打ち/ドラフト）

3. **Phase 1: 書類自動抽出機能**（v1.4.0）
   - `/company` 画面の「書類アップロード」タブ
   - 対応書類: 登記簿謄本（履歴事項全部証明書）、決算書・財務諸表
   - PDF.jsによるクライアント側テキスト抽出
   - 正規表現ベースの情報パーシング（商号、住所、資本金、設立日、代表者名など）
   - 抽出結果のプレビュー＆プロフィール反映機能
...
- **2026-01-22 (v1.4.3)**: PWA対応完了: Service Worker登録、_routes.json修正でSW/manifest静的配信、管理画面完全モバイル対応済み（ハンバーガー+ボトムナビ+サイドバー）
- **2026-01-22 (v1.4.2)**: 運用チェックダッシュボード追加: /admin/ops（L1/L2/L3スコア、30分検証チェックリスト、キュー健全性、今日のイベント、失敗/無駄可視化）
