# 情報の泉 補助金データ

## 概要
「情報の泉」から取得した補助金・助成金のデータです。
将来的にこれらのデータを`subsidy_cache`に格納し、補助金マッチングに活用します。

## ディレクトリ構成

```
data/izumi/
├── README.md                        # このファイル
├── izumi_support_urls_*.csv         # URLリスト（約17,000件）
└── details/                         # 詳細情報ファイル
    └── izumi_detail_200.csv         # 詳細取得済み（199件）
```

## データの紐付け構造

```
┌─────────────────────────────────────────────────────────────┐
│  izumi_support_urls_*.csv (約17,000件)                      │
│  ├─ policy_id: 情報の泉での補助金ID（一意キー）              │
│  ├─ primary_url: 主要な情報源URL                            │
│  └─ all_urls: 関連URL一覧                                   │
└─────────────────────────────────────────────────────────────┘
              │
              │ policy_id で紐付け
              ▼
┌─────────────────────────────────────────────────────────────┐
│  details/izumi_detail_200.csv (199件)                       │
│  ├─ policy_id: 紐付けキー                                   │
│  ├─ title: 補助金タイトル                                   │
│  ├─ max_amount: 補助上限額                                  │
│  ├─ difficulty: 難易度（★5段階評価）                       │
│  └─ support_urls_all: 申請関連URL                           │
└─────────────────────────────────────────────────────────────┘
              │
              │ discovery_items / subsidy_cache へ昇格
              ▼
┌─────────────────────────────────────────────────────────────┐
│  D1 Database                                                │
│  ├─ discovery_items: ステージング（検証前）                 │
│  └─ subsidy_cache: 本番（マッチング対象）                   │
└─────────────────────────────────────────────────────────────┘
```

---

## 1. URLリスト（izumi_support_urls_*.csv）

### ファイル一覧

| ファイル名 | レコード数 | policy_id範囲 | サイズ |
|-----------|-----------|---------------|--------|
| izumi_support_urls_500.csv | 499件 | 56 ~ 12,459 | 127KB |
| izumi_support_urls_1000.csv | 499件 | 12,460 ~ 19,860 | 110KB |
| izumi_support_urls_2000.csv | 499件 | 19,861 ~ 28,599 | 108KB |
| izumi_support_urls_2500.csv | 499件 | 28,600 ~ 36,471 | 115KB |
| izumi_support_urls_3000.csv | 499件 | 36,472 ~ 41,790 | 94KB |
| izumi_support_urls_3500.csv | 499件 | 41,791 ~ 46,600 | 105KB |
| izumi_support_urls_4000.csv | 499件 | 46,601 ~ 50,347 | 100KB |
| izumi_support_urls_4500.csv | 499件 | 50,348 ~ 53,857 | 105KB |
| izumi_support_urls_5000.csv | 499件 | 53,858 ~ 56,188 | 97KB |
| izumi_support_urls_5500.csv | 499件 | 56,189 ~ 58,919 | 105KB |
| izumi_support_urls_6000.csv | 499件 | 58,920 ~ 61,710 | 100KB |
| izumi_support_urls_6500.csv | 499件 | 61,711 ~ 65,305 | 95KB |
| izumi_support_urls_7000.csv | 499件 | 65,306 ~ 69,338 | 86KB |
| izumi_support_urls_7500.csv | 499件 | 69,339 ~ 73,491 | 85KB |
| izumi_support_urls_8000.csv | 499件 | 73,492 ~ 77,132 | 89KB |
| izumi_support_urls_8500.csv | 499件 | 77,133 ~ 80,170 | 88KB |
| izumi_support_urls_9000.csv | 499件 | 80,171 ~ 82,344 | 93KB |
| izumi_support_urls_9500.csv | 499件 | 82,345 ~ 84,186 | 87KB |
| izumi_support_urls_10000.csv | 499件 | 84,187 ~ 85,487 | 86KB |
| izumi_support_urls_10500.csv | 499件 | 85,488 ~ 86,526 | 91KB |
| izumi_support_urls_11000.csv | 499件 | 86,527 ~ 87,824 | 93KB |
| izumi_support_urls_11500.csv | 499件 | 87,825 ~ 89,941 | 90KB |
| izumi_support_urls_12000.csv | 499件 | 89,942 ~ 92,084 | 97KB |
| izumi_support_urls_12500.csv | 499件 | 92,085 ~ 93,478 | 96KB |
| izumi_support_urls_13000.csv | 499件 | 93,479 ~ 95,325 | 101KB |
| izumi_support_urls_13500.csv | 499件 | 95,326 ~ 97,312 | 114KB |
| izumi_support_urls_14000.csv | 499件 | 97,313 ~ 99,427 | 111KB |
| izumi_support_urls_14500.csv | 499件 | 99,428 ~ 102,574 | 110KB |
| izumi_support_urls_15000.csv | 499件 | 102,575 ~ 105,852 | 95KB |
| izumi_support_urls_15500.csv | 499件 | 105,853 ~ 107,790 | 109KB |
| izumi_support_urls_16000.csv | 499件 | 107,791 ~ 109,653 | 105KB |
| izumi_support_urls_16500.csv | 499件 | 109,654 ~ 111,318 | 101KB |
| izumi_support_urls_17000.csv | 499件 | 111,319 ~ 113,529 | 99KB |
| izumi_support_urls_18000.csv | 499件 | 113,530 ~ 116,297 | 100KB |
| izumi_support_urls_18066.csv | 66件 | 116,298 ~ 116,453 | 16KB |

**合計: 約17,032件の補助金URL（最終）**

### データ形式

```csv
policy_id,primary_url,all_urls
"56","https://example.com/doc.pdf","https://example.com/page.html | https://example.com/doc.pdf"
```

| カラム | 説明 |
|--------|------|
| `policy_id` | 情報の泉での補助金ID（一意キー） |
| `primary_url` | 主要な情報源URL（PDFまたはHTML） |
| `all_urls` | 関連する全てのURL（`|`区切り） |

### URLの種類

- **PDF**: 補助金の詳細資料、申請書類など
- **HTML**: 自治体の補助金案内ページ

---

## 2. 詳細情報（details/izumi_detail_200.csv）

### 概要
情報の泉の詳細ページから取得した補助金データ。
`policy_id`で上記のURLリストと紐付け可能。

### ファイル

| ファイル名 | レコード数 | policy_id範囲 | サイズ |
|-----------|-----------|---------------|--------|
| izumi_detail_200.csv | 199件 | 28 ~ 3,699 | 87KB |
| izumi_detail_400.csv | 199件 | 3,700 ~ 10,081 | 83KB |
| izumi_detail_600.csv | 199件 | 10,095 ~ 14,449 | 81KB |
| izumi_detail_800.csv | 199件 | 14,488 ~ 17,941 | 76KB |
| izumi_detail_1000.csv | 199件 | 19,865 ~ 22,954 | 80KB |
| izumi_detail_1400.csv | 199件 | 22,958 ~ 24,532 | 79KB |
| izumi_detail_1600.csv | 199件 | 24,534 ~ 26,065 | 77KB |
| izumi_detail_1800.csv | 199件 | 26,078 ~ 27,752 | 80KB |
| izumi_detail_2000.csv | 199件 | 27,754 ~ 28,597 | 77KB |
| izumi_detail_2200.csv | 199件 | 28,599 ~ 31,818 | 80KB |
| izumi_detail_2400.csv | 199件 | 31,825 ~ 34,813 | 83KB |
| izumi_detail_2600.csv | 199件 | 34,823 ~ 37,968 | 73KB |
| izumi_detail_2800.csv | 199件 | 37,975 ~ 39,925 | 70KB |
| izumi_detail_3000.csv | 199件 | 39,927 ~ 41,790 | 73KB |
| izumi_detail_3200.csv | 199件 | 41,801 ~ 43,686 | 76KB |
| izumi_detail_3400.csv | 199件 | 43,688 ~ 45,650 | 74KB |
| izumi_detail_3600.csv | 199件 | 45,652 ~ 47,451 | 73KB |
| izumi_detail_3800.csv | 199件 | 47,453 ~ 48,768 | 73KB |
| izumi_detail_4000.csv | 199件 | 48,801 ~ 50,347 | 71KB |
| izumi_detail_4200.csv | 199件 | 50,368 ~ 52,063 | 78KB |
| izumi_detail_4400.csv | 199件 | 52,066 ~ 53,306 | 76KB |
| izumi_detail_4600.csv | 199件 | 53,325 ~ 54,280 | 70KB |
| izumi_detail_4800.csv | 199件 | 54,281 ~ 55,309 | 72KB |
| izumi_detail_5000.csv | 199件 | 55,310 ~ 56,188 | 72KB |
| izumi_detail_5200.csv | 199件 | 56,195 ~ 57,253 | 74KB |
| izumi_detail_5400.csv | 199件 | 57,254 ~ 58,274 | 76KB |
| izumi_detail_5800.csv | 199件 | 59,352 ~ 60,532 | 74KB |
| izumi_detail_6000.csv | 199件 | 60,551 ~ 61,710 | 72KB |
| izumi_detail_6200.csv | 199件 | 61,711 ~ 63,056 | 76KB |
| izumi_detail_6400.csv | 199件 | 63,062 ~ 64,498 | 68KB |

**合計: 5,970件の詳細データ（継続取得中）**

### データ形式

```csv
policy_id,detail_url,title,issuer,area,publish_date,period,max_amount,difficulty,start_fee,success_fee,support_url,support_urls_all,error
"28","https://j-izumi.com/policy/28/detail","小規模事業者経営改善資金等利子補給制度","","","","","","★☆☆☆☆","要相談","要相談","https://...","https://...",""
```

### カラム定義

| カラム | 説明 | 例 |
|--------|------|-----|
| `policy_id` | 紐付けキー（情報の泉ID） | `28` |
| `detail_url` | 情報の泉詳細ページ | `https://j-izumi.com/policy/28/detail` |
| `title` | 補助金タイトル | `小規模事業者経営改善資金等利子補給制度` |
| `issuer` | 実施機関 | （空の場合あり） |
| `area` | 対象地域 | （空の場合あり） |
| `publish_date` | 公開日 | （空の場合あり） |
| `period` | 申請期間 | （空の場合あり） |
| `max_amount` | 補助上限額 | `30万円`, `10億円` |
| `difficulty` | 難易度（5段階） | `★☆☆☆☆` ~ `★★★★★` |
| `start_fee` | 着手金 | `要相談` |
| `success_fee` | 成功報酬 | `要相談` |
| `support_url` | 主要申請URL | PDF/HTMLリンク |
| `support_urls_all` | 全申請関連URL | `|`区切りで複数 |
| `error` | 取得エラー | エラー時にメッセージ |

### 難易度の目安

| 難易度 | 補助額の傾向 |
|--------|-------------|
| `★☆☆☆☆` | 数万円～50万円程度 |
| `★★☆☆☆` | 50万円～200万円程度 |
| `★★★☆☆` | 200万円～1,000万円程度 |
| `★★★★☆` | 1,000万円～5,000万円程度 |
| `★★★★★` | 5,000万円以上（大規模） |

---

## 3. DBテーブルへのマッピング

### subsidy_cache へのマッピング

```sql
INSERT INTO subsidy_cache (
  id,                        -- 'izumi-{policy_id}'
  source,                    -- 'izumi'
  title,                     -- title
  subsidy_max_limit,         -- max_amount をパース（30万円 → 300000）
  target_area_search,        -- area または support_url から推定
  detail_json,               -- 元データJSON全体
  cached_at,                 -- 現在時刻
  expires_at                 -- 30日後
) VALUES (...);
```

### discovery_items へのマッピング

```sql
INSERT INTO discovery_items (
  id,                        -- UUID
  dedupe_key,                -- 'izumi:{policy_id}'
  source_id,                 -- 'src-izumi'
  source_type,               -- 'scrape'
  title,                     -- title
  url,                       -- detail_url
  detail_url,                -- support_url
  subsidy_amount_max,        -- max_amount をパース
  stage,                     -- 'raw' (検証前)
  raw_json                   -- 元データJSON全体
) VALUES (...);
```

---

## 推奨データ抽出アーキテクチャ

### フェーズ1: jGrants API（優先）
現在実装済み。国の補助金データをAPIで取得。

### フェーズ2: 情報の泉（Playwright + Firecrawl）

```
┌─────────────────────────────────────────────────────────────┐
│  別サーバー (AWS Lambda / EC2 / 専用サーバー)                │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  1. Playwright: 情報の泉にログイン                          │
│     ├─ /search-subsidy?per_page=100 GET                    │
│     ├─ meta[name="csrf-token"] を取得                       │
│     └─ Cookie維持                                           │
│                                                             │
│  2. Playwright: POST /policy/detail ループ                  │
│     ├─ form-urlencoded形式                                  │
│     ├─ JSONの html をパース                                 │
│     └─ 「支援URL」を抽出                                    │
│                                                             │
│  3. Firecrawl: 支援URLの本文取得                            │
│     ├─ 自治体サイトのHTML取得                               │
│     ├─ PDF URLの場合は Vision API へ                       │
│     └─ 補助金詳細情報を抽出                                 │
│                                                             │
└─────────────────────────────────────────────────────────────┘
                          │
                          ▼
┌─────────────────────────────────────────────────────────────┐
│  Cloudflare Workers API                                     │
├─────────────────────────────────────────────────────────────┤
│  POST /api/admin-ops/izumi/ingest                          │
│  └─ subsidy_cache に格納                                    │
└─────────────────────────────────────────────────────────────┘
```

### なぜ Playwright + Firecrawl の組み合わせか

| 処理 | ツール | 理由 |
|------|--------|------|
| ログイン・セッション維持 | **Playwright** | Cookie管理が得意、フォーム操作が安定 |
| CSRF対策ページ取得 | **Playwright** | 動的トークン取得が必要 |
| POST APIループ | **Playwright** | セッション維持が必要 |
| 自治体サイト本文取得 | **Firecrawl** | 静的HTML取得が高速 |
| PDF解析 | **Vision API** | 構造化データ抽出 |

### 注意点

- Firecrawlは「ログインフォーム→セッション維持→POST連打」は苦手
- Playwrightは得意（セッション・Cookie維持が簡単）
- 役割分担が重要

## 活用方法

1. **このCSVの支援URLを直接Firecrawlで取得** → 情報の泉へのログイン不要
2. **source_registry登録**: 新しいsourceとして`izumi`を追加
3. **subsidy_cache格納**: 抽出したデータを正規化して格納

---

## 4. 注意事項

- URLは時間経過で無効になる可能性があります
- PDFファイルは更新される場合があります
- 自治体サイトのリダイレクトに注意
- 詳細データの空欄は情報の泉で未登録のもの

---

## 5. 更新履歴

### 2026-01-30
- **詳細データ追加**: `details/izumi_detail_3600.csv`（199件）
  - policy_id: 45,652 ~ 47,451 の範囲
- **詳細データ追加**: `details/izumi_detail_3200.csv`（199件）
  - policy_id: 41,801 ~ 43,686 の範囲
- **詳細データ追加**: `details/izumi_detail_3400.csv`（199件）
  - policy_id: 43,688 ~ 45,650 の範囲
- **詳細データ追加**: `details/izumi_detail_2800.csv`（199件）
  - policy_id: 37,975 ~ 39,925 の範囲（欠番補完）
- **詳細データ追加**: `details/izumi_detail_3000.csv`（199件）
  - policy_id: 39,927 ~ 41,790 の範囲
- **詳細データ追加**: `details/izumi_detail_2600.csv`（199件）
  - policy_id: 34,823 ~ 37,968 の範囲
- **詳細データ追加**: `details/izumi_detail_2400.csv`（199件）
  - policy_id: 31,825 ~ 34,813 の範囲
- **詳細データ追加**: `details/izumi_detail_2200.csv`（199件）
  - policy_id: 28,599 ~ 31,818 の範囲
- **詳細データ追加**: `details/izumi_detail_2000.csv`（199件）
  - policy_id: 27,754 ~ 28,597 の範囲
- **詳細データ追加**: `details/izumi_detail_1800.csv`（199件）
  - policy_id: 26,078 ~ 27,752 の範囲
- **詳細データ追加**: `details/izumi_detail_1600.csv`（199件）
  - policy_id: 24,534 ~ 26,065 の範囲
- **詳細データ追加**: `details/izumi_detail_1400.csv`（199件）
  - policy_id: 22,958 ~ 24,532 の範囲
- **詳細データ追加**: `details/izumi_detail_1000.csv`（199件）
  - policy_id: 19,865 ~ 22,954 の範囲
- **詳細データ追加**: `details/izumi_detail_800.csv`（199件）
  - policy_id: 14,488 ~ 17,941 の範囲
- **詳細データ追加**: `details/izumi_detail_600.csv`（199件）
  - policy_id: 10,095 ~ 14,449 の範囲
- **詳細データ追加**: `details/izumi_detail_400.csv`（199件）
  - policy_id: 3,700 ~ 10,081 の範囲
- **詳細データ追加**: `details/izumi_detail_200.csv`（199件）
  - policy_id: 28 ~ 3,699 の範囲
- **合計3,383件の詳細データ**を取得完了
- **README大幅更新**: PDF問題、データソース別戦略、Firecrawl限界と代替案を追記

### 2026-01-29
- **最終インポート**（16500, 17000, 18000, 18066 → **約17,000件完了**）
- 追加インポート（15000, 15500, 16000 → 約15,500件に）
- 追加インポート（13500, 14000, 14500 → 約14,000件に）
- 追加インポート（12000, 12500, 13000 → 約12,500件に）
- 追加インポート（10500, 11000, 11500 → 約11,000件に）
- 追加インポート（9000, 9500, 10000 → 約9,500件に）
- 追加インポート（8000, 8500 → 約8,000件に）
- 追加インポート（6500, 7000, 7500 → 約7,000件に）
- 追加インポート（5000, 5500, 6000 → 約5,500件に）
- 追加インポート（4000, 4500 → 約4,000件に）
- 初回インポート（約3,000件）

---

## 6. 補助金データ更新計画

### データソースの分類

| ソース | API有無 | 更新方法 | 優先度 |
|--------|---------|----------|--------|
| **jGrants** | ✅ あり | API定期取得（自動化済み） | 高 |
| **情報の泉** | ❌ なし | Playwright + スクレイピング | 高 |
| **J-Net21** | ✅ RSS | RSS定期取得（**実装済み**） | 中 |

### J-Net21 RSSフィードについて

**URL**: `https://j-net21.smrj.go.jp/snavi/support/support.xml`

**特徴**:
- 補助金・助成金・融資の最新情報をRSSで配信
- 都道府県コード付き（`dc:coverage` で `JP-01`=北海道, `JP-13`=東京都 等）
- 更新頻度: 毎日
- 運営: 中小企業基盤整備機構

**データ例**:
```xml
<item>
  <title>【茅野市】補助金・助成金：「製造業等の労務環境改善...」</title>
  <link>https://j-net21.smrj.go.jp/snavi/articles/174825</link>
  <dc:coverage>
    <rdf:value>JP-20</rdf:value>
    <rdf:label>長野県</rdf:label>
  </dc:coverage>
</item>
```

**情報の泉との関係**:
- J-Net21は「ニュースサイト」ではなく「補助金情報ポータル」
- 情報の泉と一部重複する可能性あり（同じ補助金を収録）
- RSSなので自動取得が容易（スクレイピング不要）
| **自治体個別** | ❌ なし | 個別対応（手動/半自動） | 低 |

### jGrants との関係

```
┌─────────────────────────────────────────────────────────────┐
│  jGrants API（国の補助金）                                   │
│  ├─ 自動取得: ✅ 実装済み                                   │
│  ├─ 更新頻度: 毎日                                          │
│  └─ subsidy_cache に直接格納                                │
└─────────────────────────────────────────────────────────────┘
              ↓ 一部重複
┌─────────────────────────────────────────────────────────────┐
│  情報の泉（地方自治体の補助金を含む）                        │
│  ├─ 自動取得: ❌ 要Playwright                               │
│  ├─ 価値: jGrantsに無い地方補助金を特定可能                 │
│  └─ 重複: jGrantsと一部重複（国の補助金）                   │
└─────────────────────────────────────────────────────────────┘
```

### 情報の泉データの活用戦略

1. **jGrants未掲載の補助金を特定**
   - 情報の泉の `policy_id` と jGrants の ID をマッピング
   - マッピングできないもの = jGrants未掲載 = 価値が高い

2. **地方自治体独自の補助金をカバー**
   - 情報の泉は市区町村レベルの補助金を多数収録
   - jGrants は国・都道府県レベルが中心

3. **新規補助金の検知**
   - 情報の泉の `policy_id` は新規追加時に増加
   - 定期的に最新 `policy_id` をチェック

### 今後の実装計画

#### Phase 1: 現状把握（完了）
- [x] izumi_support_urls_*.csv: 約17,000件のURL収集
- [x] izumi_detail_*.csv: 3,383件の詳細取得（継続中）

#### Phase 2: 差分検知システム
- [ ] 情報の泉の最新 `policy_id` を定期チェック
- [ ] 新規追加分を自動検知してアラート
- [ ] Playwright スクリプトで詳細取得を半自動化

#### Phase 3: jGrants との重複排除
- [ ] タイトル・金額・対象地域で類似度マッチング
- [ ] 重複フラグを付与して管理
- [ ] jGrants未掲載の補助金を `source='izumi-unique'` として区別

#### Phase 4: 自動更新パイプライン
- [ ] AWS Lambda / EC2 で Playwright 実行環境構築
- [ ] 週次で新規補助金を自動取得
- [ ] subsidy_cache への自動格納

### 課題と検討事項（詳細）

#### 1. support_url の種類と対応方法

| URLパターン | 例 | 課題 | 対応策 |
|-------------|-----|------|--------|
| **自治体HTML** | `city.xxx.lg.jp/.../page.html` | ページ構造がバラバラ | サイト別パーサー or LLM抽出 |
| **PDF直リンク** | `city.xxx.lg.jp/.../file.pdf` | 期限切れ後に内容変更 | PDF定期ダウンロード + AI解析 |
| **PDF内の情報** | 要綱、チラシ等 | 年度更新で別ファイルになる | 親ページを監視してPDF URL更新 |

#### 2. PDF問題の詳細

```
┌─────────────────────────────────────────────────────────────┐
│  PDF直リンクの問題                                           │
├─────────────────────────────────────────────────────────────┤
│                                                             │
│  例: https://city.xxx.lg.jp/files/r6_hojo_youkou.pdf        │
│                                                             │
│  ❌ 問題点:                                                  │
│  ├─ 年度更新で r7_hojo_youkou.pdf に変わる                  │
│  ├─ PDFの中身だけ見ても「今有効か」不明                      │
│  └─ 古いPDF URLがリンク切れになる                           │
│                                                             │
│  ✅ 対応策:                                                  │
│  ├─ 親HTML（案内ページ）を監視                              │
│  ├─ 親HTMLから最新PDF URLを都度取得                         │
│  └─ PDF内容 + 親HTML内容 の両方をチェック                   │
│                                                             │
└─────────────────────────────────────────────────────────────┘
```

#### 3. データソース別の取得戦略

| ソース種別 | 取得方法 | 更新頻度 | 精度 | コスト |
|-----------|----------|----------|------|--------|
| **jGrants API** | REST API | 毎日 | ◎ 高 | ○ 低 |
| **RSS/Atom** | フィード取得 | 毎日 | ○ 中 | ○ 低 |
| **自治体HTML** | スクレイピング | 週次 | △ 中 | △ 中 |
| **PDF** | ダウンロード + AI解析 | 月次 | △ 中 | ✕ 高 |

#### 4. Firecrawl の限界と代替案

| ツール | 得意 | 苦手 | 用途 |
|--------|------|------|------|
| **Firecrawl** | 静的HTML取得 | 認証、PDF、構造解析 | 初期データ収集 |
| **Playwright** | 認証、動的JS、フォーム | 大量処理の速度 | ログイン必須サイト |
| **Puppeteer** | 認証、動的JS | 環境構築が複雑 | 高速並列処理 |
| **Jina Reader** | Markdown変換 | 日本語サイト | 本文抽出 |
| **LLM + Vision** | PDF解析、構造理解 | コスト | PDF内容抽出 |

#### 5. 実装優先度の整理

```
優先度: 高 ───────────────────────────────────────────────> 低
 
[jGrants API]  [RSS/J-Net21]  [自治体HTML]  [PDF解析]
     ✅             ✅              🚧            📋
   実装済み       実装済み        要実装       将来対応
```

#### 6. 具体的な課題リスト

| # | 課題 | 詳細 | 対応案 |
|---|------|------|--------|
| 1 | **ログイン認証** | 情報の泉はログイン必須 | Playwright + 認証情報の安全管理 |
| 2 | **PDF期限問題** | 年度更新でURL変更 | 親HTMLの監視 + 差分検知 |
| 3 | **サイト構造多様性** | 自治体ごとにHTML構造が異なる | LLM抽出 or サイト別パーサー |
| 4 | **更新検知** | 新規補助金・変更の検知 | policy_id監視 + コンテンツハッシュ |
| 5 | **データ品質** | 空欄フィールドの補完 | 支援URLから追加抽出 |
| 6 | **重複判定** | jGrantsとの類似度 | タイトル類似度 + 金額 + 地域 |
| 7 | **コスト管理** | PDF解析のAPI費用 | バッチ処理 + キャッシュ活用 |

---

## 7. 次のステップ

1. **詳細データの継続取得**: policy_id 63,056 以降のデータを順次取得
2. **support_url のパターン分析**: HTML/PDF の比率と特徴を調査
3. **jGrants重複チェック**: 既存796件とjGrantsデータの照合
4. **親HTML監視の仕組み**: PDF直リンクではなく案内ページを監視
5. **discovery_items への登録**: 検証パイプラインの構築
6. **補助金検索API対応**: `source='izumi'` でフィルタリング可能に
