...
- [x] izumi_detail_*.csv: 597件の詳細取得（継続中）
...
| 課題 | 検討内容 |
|------|----------|
| **ログイン認証** | 情報の泉はログイン必須。認証情報の安全な管理 |
| **スクレイピング頻度** | サーバー負荷を考慮した適切な間隔設定 |
| **データ品質** | 空欄が多いフィールドの補完方法 |
| **重複判定** | jGrants との類似度閾値の設定 |
| **URL有効性** | 自治体サイトのURL変更への対応 |

---

## 7. 次のステップ

1. **詳細データの継続取得**: policy_id 14,449 以降のデータを順次取得
2. **jGrants重複チェック**: 既存597件とjGrantsデータの照合
3. **discovery_items への登録**: 検証パイプラインの構築
4. **subsidy_cache への昇格**: 品質検証後にマッチング対象化
5. **補助金検索API対応**: `source='izumi'` でフィルタリング可能に
